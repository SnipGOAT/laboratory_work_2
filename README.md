...
# Алгоритм нормализации ФИО

Ключевая проблема при объединении данных из двух источников — ФИО одного и того же человека могут быть записаны по-разному: в первом источнике «Иванов Иван Иванович» (полное, с заглавными буквами), во втором — «иванов иван иванович» (строчные), а в другом варианте — «Козлов Алексей» против «Алексей Козлов» (разный порядок слов). Чтобы корректно сопоставить такие записи, разработан алгоритм нормализации.

## Алгоритм (функция `normalize_name`):

1. Удаляются все знаки препинания и символы, не являющиеся буквами или пробелами (регулярное выражение `[^\w\s]`).
2. Вся строка приводится к нижнему регистру.
3. Строка разбивается на отдельные слова (по пробелам).
4. Слова сортируются в алфавитном порядке.
5. Отсортированные слова склеиваются обратно через пробел.

Результат — каноническая строка, не зависящая от регистра, порядка слов и лишних символов.
